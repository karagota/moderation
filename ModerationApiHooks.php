        ...
        APIAfterExecuteHook
{
     ...
     *
     * Modify response when uploaded file is sent to moderation
     * @param  $module
     */
    public function onAPIAfterExecute($module): void
    {
        ...
        if (!$services->getService('Moderation.CanSkip')->canUploadSkip($user)) {
            // Добавляем поле модерации
            $apiResult->addValue(null, 'moderation', 'pending');

            // Добавляем временный URL для превью
            $stashUrlBase = $GLOBALS['wgUploadStash']['stashDirectoryURL'] ?? '/w/images/stash';

            // Для успешных загрузок
            if (isset($resultData['upload']['filekey'])) {
                $filekey = $resultData['upload']['filekey'];
                $apiResult->addValue(
                    null,
                    'moderationTempUrl',
                    $stashUrlBase . '/' . $filekey
                );
            }
            // Для ошибок модерации
            elseif (isset($resultData['errors'][0]['data']['filekey'])) {
                $filekey = $resultData['errors'][0]['data']['filekey'];
                $apiResult->addValue(
                    ['errors', 0, 'data'],
                    'moderationTempUrl',
                    $stashUrlBase . '/' . $filekey
            ...
            // Устанавливаем HTTP статус 202 (Accepted)
            RequestContext::getMain()->getOutput()->setStatusCode(202);
        }
    }
}
